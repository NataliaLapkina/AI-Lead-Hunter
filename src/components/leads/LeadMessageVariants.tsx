    ...
    <div className="space-y-3 rounded-lg border bg-muted/20 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          {history.length > 0
            ? ru.leads.messageHistoryCount.replace('{count}', String(history.length))
            : ru.leads.messageHistoryEmptyShort}
        </p>
      </div>

      {history.length === 0 ? (
        <p className="text-sm text-muted-foreground">{ru.leads.messageHistoryEmpty}</p>
      ) : (
          ...
          {history.map((variant, index) => (
            <div key={variant.id} className="space-y-2 rounded-lg border bg-background p-3">
              ...
              <Button
                ...
                className="gap-2"
              >
                {restoringId === variant.id && <Loader2 className="h-4 w-4 animate-spin" />}
